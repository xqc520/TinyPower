...
log.info("drv_psm", "当前使用的模组是：", module, "rtos", rtos.version())

local function isWifiModule()
    return module == "Air8000A"
        or module == "Air8000U"
        or module == "Air8000N"
        or module == "Air8000AB"
        or module == "Air8000W"
...
local function disableWifiChip(reason)
    if not isWifiModule() then
        return
    end
    if airlink then
        if airlink.pause then
            local ok, ret = pcall(airlink.pause, 1)
            log.info("drv_psm", reason or "wifi", "airlink pause", tostring(ok), tostring(ret))
        end
        if airlink.power then
            local ok, ret = pcall(airlink.power, false)
            log.info("drv_psm", reason or "wifi", "airlink power off", tostring(ok), tostring(ret))
        end
    end
    if pm and pm.WIFI then
        log.info("drv_psm", reason or "wifi chip off", tostring(pm.power(pm.WIFI, 0)))
    elseif gpio and gpio.setup then
        gpio.setup(23, nil, gpio.PULLDOWN)
        log.info("drv_psm", reason or "wifi gpio pulldown")
    end
end
M.disableWifiChip = disableWifiChip

if config.PSM_BOOT_DISABLE_WIFI_CHIP then
    disableWifiChip("boot wifi chip off")
end
...
local function set_psm_func_item()
    ...
    -- 不在这里提前关闭 USB；进入 WORK_MODE=3 时内核会自动关闭。
    -- 提前关闭会截断调试日志，看不到后面的 PSM+ 入口状态。
    ...
    disableWifiChip("psm wifi chip off")
    ...
    if module == "Air8000A" or module == "Air8000U" or module == "Air8000N" or module == "Air8000AB" or module == "Air8000D" or module == "Air8000DB" then
        if config.PSM_KEEP_GNSS_BACKUP then
            gpio.setup(24, 1, gpio.PULLUP)
            log.info("psm", "gnss backup on", 24)
        else
            gpio.setup(24, nil, gpio.PULLDOWN)
            log.info("psm", "gnss backup off", 24)
        end
    end
...
local function psm_task()
    ...
    if config.PSM_ENABLE_GSENSOR_WAKEUP
        and tonumber(string.sub(rtos.version(), 2)) < 2027
        and (module == "Air8000A" or module == "Air8000U" or module == "Air8000N" or module == "Air8000AB" or module == "Air8000D" or module == "Air8000DB")
    then
    ...
    log.info("psm_task", "wakeup config done")
    ...
    -- 一定要仔细阅读这个函数的代码注释说明，根据自己的项目需求来决定是否需要配置每一项功能
    set_psm_func_item()
    log.info("psm_task", "func config done")
    ...
    log.info("psm_task", "enter WORK_MODE 3")
    local ok = pm.power(pm.WORK_MODE, 3)
    log.info("psm_task", "WORK_MODE 3 ret", tostring(ok))
